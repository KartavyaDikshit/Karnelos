...
- [x] SMP infrastructure module (`smp.rs`) with `cpu_count()` API
- [x] ACPI module (`acpi.rs`) with RSDP scan, EBDA/BiOS ROM search
- [x] MADT find/parse for APIC and CPU topology discovery
- [x] Single-core fallback when ACPI is unavailable
- [ ] AP startup via SIPI with real-mode trampoline
- [ ] Per-CPU data structures (GDT, IDT, TSS, kernel stack)
...
| 8 - SMP + Scheduling | 3-4 weeks | Phase 5 | 🔶 In progress |
